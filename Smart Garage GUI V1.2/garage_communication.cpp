...
garage_t * garage_new(int com_port)
{
    ...
    if(server == NULL)
        return NULL;
    garage_t * garage = (garage_t*)malloc(sizeof(garage_s));
...
static char send_receive_cycle(garage_t * garage, char message)
{
    ...
    serial_server_send_bytes(garage->server, send_message, 1);
    char buffer[READ_BUFFER_SIZE];
    serial_server_read_bytes(garage->server, buffer, READ_BUFFER_SIZE);
    return buffer[0];
}
