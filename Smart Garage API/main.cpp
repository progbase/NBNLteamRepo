...
int main(void)
{
    ...
    //garage_t * MYgarage = garage_new(PORT);

    Image garage;
    garage.loadFromFile("garage.jpg");
    Texture garage_t;
    garage_t.loadFromImage(garage);
    Sprite garage_s;
    garage_s.setTexture(garage_t);
    garage_s.setScale(2.74, 1.54);
        ...
        if (event.type == Event::MouseButtonPressed)
        {
            if (event.mouseButton.button == Mouse::Left)
            {
                if(event.mouseButton.x > 319 && event.mouseButton.x < 517 && event.mouseButton.y > 426 && event.mouseButton.y < 539)
                    {
                        //garage_set_door_close(MYgarage);
                        //Sleep(SLEEP_TIME);
                    }
            }
        }
